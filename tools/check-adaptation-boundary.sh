...
check_codex_bin_wrappers() {
  ...
  if ! grep -Fq 'validate_dispatch_inputs' adapters/codex/bin/dispatch-headless.py \
    ...
    || ! grep -Fq 'resolve_qa' adapters/codex/bin/dispatch-headless.py \
    || ! grep -Fq 'invalid-dispatch-qa' utilities/dispatch_mode_contract.py \
    || ! grep -Fq '"quick", "light", "standard", "thorough", "adversarial"' utilities/dispatch_mode_contract.py \
...
check_codex_tool_projection() {
  ...
  TOOL_DEFERRED="$TOOL_DEFERRED check-sd-procedure-hooks.py check_sd_procedure_hooks.test.py sd-procedure-hooks.tsv stage-advance-census.py stage_advance_census.test.py dispatch-refusal-census.py dispatch_refusal_census.test.py check_ledger_invariant.py ledger_invariant.test.py check-surface-budget.py check_surface_budget.test.py surface-budget.json bytecode-cache-tolerance.test.sh worktree-lock.sh argparse_abbrev_guard.test.py"
  tool_count=0
  for f in tools/*; do
    [ -e "$f" ] || continue
...
check_opencode_bin_wrappers() {
  ...
  if ! grep -Fq 'validate_preflight' adapters/opencode/bin/dispatch-headless.py \
    ...
    || ! grep -Fq 'resolve_qa' adapters/opencode/bin/dispatch-headless.py \
    || ! grep -Fq 'invalid-dispatch-qa' utilities/dispatch_mode_contract.py \
    || ! grep -Fq '"quick", "light", "standard", "thorough", "adversarial"' utilities/dispatch_mode_contract.py \
