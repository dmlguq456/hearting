...
12. Two assurance properties are intensity-independent: (a) every review that actually runs carries the refute-by-default adversarial **stance** of §1.1; (b) an independent pass declares and records its actual independence axes. Cross-harness remains primary, while model-profile and perspective asymmetry generalize dual-model direction exploration into bounded N-way groups. Only the registry may select or widen a group, and neither property converts `direct` into added sessions or widens quick's work graph — quick's direction-setting is a separately declared layer ahead of the route, not a widened group inside it.
