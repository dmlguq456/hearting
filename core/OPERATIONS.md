...
The frame pair runs in the depth-0 bootstrap layer ahead of the route
(`WORKFLOW §0.4`), so depth-0 launches it: one Bash call per leg — anchor
`frame` and sibling `frame-alternative`, each on a different sealed candidate
harness — or both on the one supported harness when only one is, which the
route seals as `harness_diversity: single-harness:<h>` on both nodes and the
direction card's cost field reports.
...
- **One Bash call per leg, never two legs in one call.** `hooks/dispatch-owner-rewake.py` arms one waiter per Bash call, oldest unclaimed depth-1 row first (`registry_launch`'s docstring states it). Both legs in one call leaves the second waiter unarmed and its direction question silently fails to reach the user — the symptom this layer exists to remove.
- **The four artifact-scoping variables above are checked at launch.** `dispatch-owner.py` refuses a frame launch missing any of them (`frame-artifact-scope-missing:<names>`). This path runs through `dispatch-owner.py`, not `dispatch-batch.py`, so the known child-launch defect that drops three of the four misses it, and a subset would land the briefs in an unrelated, possibly closed, campaign/cycle directory. §5.10a governs launch stdout: read it whole.
- **No machine fallback chain.** A frame node declares no `fallback_hops`. When a leg's harness is unavailable rather than late, depth-0 re-launches that one leg explicitly on another sealed candidate harness from the route's own candidate list, and records that it did so.
- **One-time demotion.** If the anchor sealed `top` and its launch fails with a rate-limit-shaped typed refusal, or exits early with zero artifacts, re-launch that leg once at `deep` — exactly once, and never a second `top` attempt after it. Record `frame_profile_degraded=top→deep` in both the route/attempt record and the user-facing card or notice for that launch. The input is the launch outcome you observed, never a usage query: the top tier has no local remaining-quota surface to read.

**Join, then interview.** No path proceeds on one leg: the first work node's `depends_on` names both, and `completion_marker_gate` refuses its start until both hold current markers. A late leg is waited for to the dispatch watchdog's hard limit; past it depth-0 starts nothing, says which leg is missing and why, and asks. Two differing direction verdicts are put side by side, and nothing downstream starts until the user picks one. Then `WORKFLOW §0.4`'s interview runs — including the restatement's explicit yes/no even at zero questions, which `understanding_confirmed` records — followed by `frame_interview.py validate-answers` and `render-intent` into the intent path the capability's own document names (`shards/frame/intent.md` for code). Then `workflow-supervisor.py release --route <route> --gate frame-review --decision proceed --answers <file>` — the one machine event that authorizes the owner's launch. Only then does depth-0 launch the owner, passing `intent.md`'s absolute path in the prompt as `Intent:`.
