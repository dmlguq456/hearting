...
- **Export all four artifact-scoping variables above before every launch call**, never a subset. This path runs through `dispatch-owner.py`, not `dispatch-batch.py`, so the known child-launch defect that drops three of the four misses it — but only while it passes all four itself. Otherwise the briefs land in an unrelated, possibly closed, campaign/cycle directory. §5.10a governs launch stdout: read it whole.
