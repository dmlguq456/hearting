...
4. Before the first durable capability artifact, compile and bind the checked route with `preflight.sh route --capability autopilot-research ...`; `preflight.sh route autopilot-research` is grounding only, not route participation. A native-subagent restriction never authorizes direct execution.
5. Run `adapters/codex/bin/preflight.sh capability-info autopilot-research` and obey the reported status:
...
- Capability grounding only: `adapters/codex/bin/preflight.sh route autopilot-research [cwd] [session-id]`
- Before durable capability output: `adapters/codex/bin/preflight.sh route --capability autopilot-research <complete compile arguments>`
