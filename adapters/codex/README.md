...
| dispatch-owner selection | An ordinary dispatch-depth-1 owner launches through `preflight.sh dispatch-owner [--adapter <harness>] --dry-run|--register|--start`, a separate surface from the `headless dispatch` row below. It loads `profiles/dispatch-defaults.yaml` through `utilities/dispatch-owner.py`, runs the SD-22 cascade (explicit target, hard eligibility, configured `depth1_owner`, sealed recent-attempt balance, then eligibility fallback), and execs only the selected adapter's `dispatch-headless.py`; the schema-v2 normal pool includes Claude, Codex, and OpenCode, and emits bounded exact attempt counts without claiming provider capacity. The selector preserves the actual caller runtime separately from the selected owner adapter and forbids caller-supplied completion-policy or unmanaged-poll flags |
