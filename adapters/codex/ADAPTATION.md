...
| dispatch-owner selection | An ordinary dispatch-depth-1 owner uses `dispatch-owner [--adapter <harness>] --dry-run|--register|--start`, a separate mapping row from `headless dispatch` below. It delegates to the portable `utilities/dispatch-owner.py` selector so `profiles/dispatch-defaults.yaml` and the SD-22 cascade (explicit target, hard eligibility, configured `depth1_owner`, sealed recent-attempt balance, then eligibility fallback) pick the adapter before any wrapper runs; the schema-v2 normal pool includes Claude, Codex, and OpenCode, and reports bounded exact attempt counts without inventing capacity evidence. The selector execs only the chosen `adapters/<selected>/bin/dispatch-headless.py`, preserves actual caller runtime separately from selected owner adapter, and forbids completion-policy or unmanaged-poll flags |
