...
- Peer-session steering (`OPERATIONS §5.14`): watch a peer depth-0 session with the checked `utilities/peer-steward.py watch` — a detached watcher writes a disk receipt and the `PostToolUse(Bash)` `asyncRewake` carrier wakes this session once (`join`/`status`/`rearm`/`ack` read it; the next prompt sweeps anything un-acked). `wait` stays a bounded foreground surface and must never be backgrounded; `peer-steward.py start` defaults a launched child session to `bypass` permissions (`dispatch-defaults.yaml` `steward.child_permission_mode`, opt-out is `inherit`).
