...
class Depth1WorkerTypeProjectionTest(unittest.TestCase):
    ...
    def test_helper_matches_referenced_source_and_is_not_duplicated_in_adapters(self):
        current = ast.parse(MODULE_PATH.read_text(encoding="utf-8"))
        reference = ast.parse(
            subprocess.check_output(
                ["git", "show", "fb24852d:hooks/dispatch-owner-rewake.py"],
                text=True,
            )
        )
        def function(tree):
            return next(node for node in tree.body
                        ...
                        and node.name == "_worker_type_is_depth1")
        self.assertEqual(ast.dump(function(current)), ast.dump(function(reference)))
